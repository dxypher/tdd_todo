...
feature 'Manage todos' do
  ...
  scenario 'view only my todos' do
    ...
    user_sees_todo("Buy some milk")
    user_does_not_see_todo("Buy some eggs")
  end
  ...
  scenario 'mark todos as complete' do
    ...
    user_sees_completed_todo('Buy some milk.')
  end
  ...
  def user_sees_completed_todo(description)
  ...
  def user_sees_todo(description)
    expect(page).to have_css 'li.todo', text: description
  end

  def user_does_not_see_todo(description)
